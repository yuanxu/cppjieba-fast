...
class KeywordExtractorrTest(Spec):
    ...
    def setUpClass(cls):
        ...
        cls.extractor = KeywordExtractor(KeywordExtractorrTest.dt)
    ...
    class init:
        ...
        def takes_arg2_as_IDF_PATH_and_arg3_as_STOP_WORD_PATH(self):
            KeywordExtractor(self.dt, IDF, STOP_WORD)

    class extract_tags:

        def takes_arg1_as_sentence(self):
            self.extractor.extract_tags(self.sentence)

        def takes_arg2_as_topK(self):
            self.extractor.extract_tags(self.sentence, topK=5)

        def returns_list(self):
            r = self.extractor.extract_tags(self.sentence, topK=5)
